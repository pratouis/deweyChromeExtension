...
window.onload = () => {
  chrome.storage.sync.get(['options'], (result) => { //Checks the options popup.
      if (!result.options) { // If there are no options checked, app will do nothing.
        ...
        document.getElementById('modalCloseButton').addEventListener('click', () => document.getElementById('dialogModal').classList.add('modalHide'));
        window.addEventListener('keyup', (e) => {
          if (e.key === 'Escape' || e.code === 'Escape') document.getElementById('dialogModal').classList.add('modalHide');
        });
        ...
        let links = !!document.querySelector('.link:not(.promoted)') ? document.querySelectorAll('.link:not(.promoted)') : document.querySelectorAll('.scrollerItem:not(.promoted)');
        links.forEach((redditLink) => {
          ...
          if(window.location.pathname === "/"){
            ...
            if (result.options.Subreddits.includes(subreddit)) addDeweyRedditFunctionality(redditLink, title);
          } else {
